...
def main():
	...
	draw_plot(tree, test_data_points, path="latex/kdtree.tex")
...
@save_figure()
def draw_plot(tree, test_data_points, path=""):
	...
	if axis == 0 :
		...
		if branch is not None and prev_node is not None:
			if branch:
				...
				min_x = prev_node[0]
		# if cur_node is not None:
		plt.plot([min_x,max_x],[cur_node[1],cur_node[1]],linestyle='-', color='red', linewidth=1)

	if cur_node is not None:
		plt.plot(cur_node[0],cur_node[1], 'k.', alpha=0.444, c="#2222ee" )
